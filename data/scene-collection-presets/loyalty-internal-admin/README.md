...
每个 `*.json` 对应「创建列表变量 → 场景变量」里的一项 **列表（collection）** 结构；`seedValues` 为创建时写入 `payload.values` 的预览行，**须与产品截图条数一致**。

**对象（object）** 类型的专用变量（推荐订阅套餐、正向 GMV 汇总）不在此目录维护，真源为 `src/payload-contract/builtin-structure-catalog/registry.ts`。
