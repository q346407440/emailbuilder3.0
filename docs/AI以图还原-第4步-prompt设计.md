...
- button   必填 label；可选 href / tone / radius / width(fill|hug) / height(hug|relaxed)  // 默认小胶囊；通栏大条写 width:fill
...
5. 按钮不写样式，默认就是品牌 CTA 小胶囊；只有偏离时才用 tone/radius 覆盖。
   ...
   - **高度（可选，二选一）**：常规小胶囊 → 不写 height 或 height:"hug"；设计图里按钮色块**明显比标准胶囊更高**（约 1.5 倍视觉高度）→ height:"relaxed"。禁止写 heightMode、禁止写 px。
...
- 按钮：通栏大条 `width:"fill"`；小胶囊不写 width；明显偏高写 `height:"relaxed"`，常规不写 height。
