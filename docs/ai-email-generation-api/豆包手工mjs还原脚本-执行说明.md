...
豆包（AI）          → 列资产槽、写 mjs body、validate 失败时打 XML patch
```
...
- **首次生成**：`mjsMotherBody.ts` + `mjs-patch-contract` slot 锚点 → 豆包输出 **XML patch** → `parseMjsPatchesFromLlm` + `applyMjsPatches` merge；失败同轮 fallback 整段 body
- `tryPostProcessMjs` / `tryPostProcessMjsFromBody`：merge 后拼接、完整性检查；**可恢复错误**仍保留 source，进入 patch 重试
...
- 阶段：`MR:AssetSlots` → `MR:MjsGenerate`（底稿+patch 或整段 body）→ `MR:MjsPatch`（校验失败补丁）
...
flowchart TD
  ...
  C --> D[豆包 MR:MjsGenerate · 底稿+patch 或整段 body]
  D --> E[程序拼接 mjs + 完整性检查]
...
**默认（尝试 1）— 底稿 + XML slot patch：**

- 程序注入 `buildMotherMjsBody()`：助手函数 + `/* @mjs-slot:id */` 锚点占位
- 豆包**只输出** `<mjs-patches>` XML（`kind="slot"` + `id` + `<replace>` CDATA），无需 SEARCH 全文
- merge 不干净（未全命中 / 残留 XML 标记）→ 同轮 fallback 整段 body

**validate 失败 — XML search patch：**

- 同样 `<mjs-patches>` 信封，`<patch kind="search">` + `<search>` / `<replace>` CDATA
- **merge 层**：每条 search patch 在源码中 **replaceAll**（循环 `indexOf` 直到无命中），一条 patch 可一次改多处相同片段；`applied` 仍按 patch 条数计，`searchReplacements` 记录实际替换次数

**Fallback / 重试 — 整段 body：**

- `promptsMjs.ts` 整段 body 生成（原路径）

Prompt / 契约真源：`src/mjs-patch-contract/`、`promptsMjsDelta.ts`（slot）、`promptsMjsEdit.ts`（search）、`mjsPatchApply.ts`（解析/merge）
...
尝试 N+1：
  ...
  ③ 豆包 MR:MjsPatch（XML search patch）
...
| 文件 | 含义 |
|------|------|
| `00-run-meta.json` | 运行元数据：`mjsGenerateMode`、各步骤 `durationMs`/`durationSec`、总耗时 |
| `00-run-timing.txt` | 同上摘要（纯文本，便于 diff 对比 delta vs 整段） |
...
| 背景节点缺 `border` / `borderRadius` | 有背景必须显式写出；**程序 autofix** 会按 validate 路径补 `wrapperStyle.borderRadius`（含无 `backgroundColor` 的 layout） |
| 产品宫格只有灰块无图 | 阶段① **MR:AssetSlots** 未列产品缩略图 slot，或阶段② **productCard** 未传 `PEXELS.*`（勿用 gray layout「产品占位图」） |
| 摄影图/产品图高度总一样（如 240/400/100px） | mother 助手或 prompt 示例里写死了默认 px，模型照抄；应只在 **buildS* 调用处**按设计图传 height/imgWidth/imgHeight，助手内无默认 |
| `ReferenceError: instagram is not defined` | 写了 `ICON.icon-instagram`；须改为 `ICON["icon-instagram"]`（程序 autofix 也会自动改） |
| validate 报多处 `wrapperStyle.borderRadius` / `emailRoot.props.borderRadius` | 优先改 **rowLayout/gridBlock/sectionShell** 助手补 `borderRadius: { mode: 'unified', radius: '0' }`；勿全局 delete 所有 borderRadius 行；日志见 `03-validate-errors-attempt-*.json` |
