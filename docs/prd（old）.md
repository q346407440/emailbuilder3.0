...
|**画布描边宽度（四边 top/right/bottom/left）/ 样式 / 颜色**|非负像素宽度；**实线 / 虚线 / 点线**；颜色含透明；默认无描边|
|**页面内边距（四边 top/right/bottom/left）**|各边非负像素；默认 **0**|
...
|**内边距（四边 top/right/bottom/left）**|各边非负像素；默认 **0**|
|**容器背景色**|颜色（含透明）|
|**容器圆角（四角 topLeft/topRight/bottomRight/bottomLeft）**|非负像素；默认 **0**（直角）|
|**容器描边宽度（四边 top/right/bottom/left）**|非负像素；默认 **0**（无描边）|
|**容器描边样式**|**实线** / **虚线** / **点线**|
|**容器描边颜色**|颜色（含透明）；默认透明|
...
|**按钮文字样式**|**加粗** / **斜体** 开关|
|**按钮圆角**|四角分别配置（topLeft/topRight/bottomRight/bottomLeft）|
|**按钮描边**|四边分别配置宽度（top/right/bottom/left）+ 样式 + 颜色|
...
|**条带圆角**|四角分别配置（topLeft/topRight/bottomRight/bottomLeft）|
...
> **历史说明（2026-06）**：本章描述的 mjs patch 多步链路（visual blueprint → mjs 脚本 → `<mjs-patches>`）**已下线**。现行实现为 **RestoreAst 三步骤**（见 `docs/prd(new).md` 与 `docs/AI以图还原-第2-6步-管线收尾设计.md`）。下文保留作旧 PRD 归档。
