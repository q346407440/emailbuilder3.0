...
description: >-
  ...
  以 data/emails/coupon-available 为对齐样例。当用户或 Agent 新建/扩展 tokenPresets、批量绑定 $themeRef、核对模板是否引用未声明 token、或要求「与 coupon-available 同一套预设范围」时使用。
---
...
| 样例邮件 | **`data/emails/coupon-available/layouts/default/tokenPresets.json`**（与该校验通过的 nested template 对齐） |
| **template 落盘** | **`$themeRef` / `bindings.tokenPath`** 写在 nested 节点上；结构见 **`src/template-disk-contract/`** |
...
3. **选哪条标准键**：按设计意图对齐 **单一意图**（如主色 → `colors.primary`、模块竖直节奏 → `tokens.spacing.section`）；细则见各 **`family`** 语义注释（**`standard-keys.ts`** 与 **`coupon-available`**）。  
