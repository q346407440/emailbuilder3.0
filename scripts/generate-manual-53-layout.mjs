#!/usr/bin/env node
/**
 * 手工还原「购买后 1（模板 53）」IMBŌDHI — 模拟 pipeline 产物形态，不经 LLM。
...
const EMAIL = 'engagement_post_purchase_template53';
const P = 'ept53';
...
const DESIGN_DST = join(__dirname, '../public/test-assets/post-purchase-template-53.png');

const COLORS = {
  cream: '#F5F0E8',
  chocolate: '#3D2314',
  terracotta: '#B8957A',
  terracottaHeading: '#D4A574',
  white: '#FFFFFF',
};
...
function buildS1() {
  const sec = sectionShell(`${P}-s1`, '品牌头部', { bg: COLORS.cream, padTop: '24px' });
  sec.children.push(
