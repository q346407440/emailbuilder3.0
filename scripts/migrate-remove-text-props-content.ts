...
async function main() {
  ...
  for (const file of files) {
    ...
    const parsed = JSON.parse(raw) as EmailTemplate | { blocks?: EmailTemplate["blocks"] };
    if (!parsed.blocks || typeof parsed.blocks !== "object") continue;
    const template = parsed as EmailTemplate;
