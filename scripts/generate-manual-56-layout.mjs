#!/usr/bin/env node
/**
 * 手工还原「购买后 4（模板 56 · SUNDAYS）」— 不经 LLM。
 * 图源：Pexels（与 pipeline resolveAssetsFromDesign 同批 URL）；无图标块。
 */
...
const EMAIL = 'engagement_post_purchase_4_manual';
const P = 'ep4man';
...
const DESIGN_DST = join(__dirname, '../public/test-assets/post-purchase-template-56-manual.png');
...
function buildS6() {
  ...
  sec.children.push(
    ...
    textBlock(`${P}-s6-cap`, '图片说明', 'Rick Connection on YouTube talks Sundays benefits while Peanut faceplants into a bowl.', {
      fontSize: themeRef('tokens.typography.caption'),
      fontSizePath: 'tokens.typography.caption',
      color: COLORS.textGray,
      alignH: 'left',
...
function buildS10() {
  ...
  sec.children.push({
    id: `${P}-s10-stack`,
    type: 'layout',
    blockMeta: { blockType: 'layout.container', name: '按钮组' },
    props: { direction: 'vertical', gapMode: 'fixed', gap: '12px' },
    wrapperStyle: {
      contentAlign: { horizontal: 'center', vertical: 'top' },
      widthMode: 'fill',
...
function buildS11() {
  ...
  const prefBox = {
    ...
    children: [
      ...
      textBlock(`${P}-s11-pref-text`, '偏好文本', 'Get only the emails you want and none you don\'t. ', {
        fontSize: themeRef('tokens.typography.caption'),
        fontSizePath: 'tokens.typography.caption',
        color: COLORS.textGray,
      }),
...
const tokenPresets = {
  ...
  presets: {
    default: {
      label: '手工还原购买后 4（模板 56）',
      description: 'SUNDAYS 狗粮社交邀请邮件 — 手工还原；图源 Pexels（pipeline 同批 URL）。',
      tokens: {
        colors: {
...
const meta = {
  schemaVersion: '1.0.0',
  displayName: '手工还原购买后 4',
  description: '购买后 4（模板 56 · SUNDAYS）：UGC 双图、大图、拼图栅格、社交 CTA 与页脚。图源 Pexels。',
  source: 'human',
  ...
  variants: [
    {
      id: 'default',
      label: '手工还原购买后 4',
      description: 'SUNDAYS 购买后 4 — 按设计图手工还原',
      publishStatus: 'published',
    },
  ],
};
