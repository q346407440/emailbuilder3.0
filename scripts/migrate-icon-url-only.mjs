...
for (const emailId of readdirSync(EMAILS, { withFileTypes: true })
  .filter((d) => d.isDirectory())
  .map((d) => d.name)) {
  const tpl = join(EMAILS, emailId, "template.json");
  if (existsSync(tpl)) migrateFile(tpl, `${emailId}/template.json`);
}
