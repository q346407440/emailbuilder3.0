...
function main() {
  ...
  for (const [emailDir, templatePaths] of [...byEmail.entries()].sort((a, b) =>
    ...
    for (const absPath of templatePaths) {
      ...
      const template = JSON.parse(readFileSync(absPath, "utf8"));
      const result = migrateTemplate(template, values, valuePatches);
      ...
      if (WRITE) {
        writeFileSync(absPath, `${JSON.stringify(result.next, null, 2)}\n`, "utf8");
      }
    }
