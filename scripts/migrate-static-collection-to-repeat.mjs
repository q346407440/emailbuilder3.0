...
function runJob(job) {
  ...
  let result;
  if (job.kind === "grid-cells") result = migrateGridCells(template, job);
  else if (job.kind === "vertical-rows") result = migrateVerticalRows(template, job);
  else if (job.kind === "mod-cards") result = migrateModCards(template, job);
  ...
  if (WRITE) {
    writeFileSync(abs, `${JSON.stringify(result.next, null, 2)}\n`, "utf8");
  }
}
