#!/usr/bin/env node
/**
 * 生成「VIP 会员加入」学习模板到 data/emails/member-vip-join/
 */
...
const OUT_DIR = join(ROOT, "data", "emails", "member-vip-join");
...
function build() {
  ...
  const addBenefitCell = (cellId, parentId, item, collectionSlotId, index, collectionMeta) => {
    ...
    reg(iconWrapId, "layout.container", `权益图标底 ${index + 1}`, {
      ...
      bindings: { "wrapperStyle.borderRadius.radius": themeBinding("tokens.radius.cta") },
    });
    ...
    reg(iconId, "content.icon", `权益图标 ${index + 1}`, {
      ...
      props: { src: item.icon, color: item.iconColor, size: "20px", link: "" },
      bindings: {
        "props.src": {
  ...
  // --- 入会即时奖励条 ---
  const modIncentive = `${PREFIX}-mod-incentive`;
  const incentiveText = `${PREFIX}-incentive-text`;
  addMod(
    modIncentive,
    "模块 · 入会即时奖励",
    [incentiveText],
  ...
  const incentiveRuns = [
    { text: "joining as a member, you can obtain immediately: points: " },
    { text: "888", bold: true },
  ];
  reg(incentiveText, "content.text", "入会奖励说明", {
    id: incentiveText,
    type: "text",
    parentId: modIncentive,
    children: [],
    wrapperStyle: wsBase({ contentAlign: { horizontal: "center", vertical: "top" } }),
    props: {
      content: snapshotFromRuns(incentiveRuns),
      textBody: textBodyRuns(incentiveRuns),
      fontFamily: themeRef("fonts.body"),
      fontSize: themeRef("tokens.typography.body"),
      color: "#F9FAFB",
      bold: false,
  ...
  const joinBtn = `${PREFIX}-join-btn`;

  const benefitCellIds = MEMBER_BENEFITS.map((_, i) => `${PREFIX}-benefit-cell-${i + 1}`);

  addMod(
    modVipCard,
    "模块 · 会员权益卡",
    [vipLevelRow, progressText, progressBar, benefitsGrid, joinBtn],
    {
  ...
  reg(vipLevelRow, "layout.container", "VIP 等级行", {
    ...
    type: "layout",
    parentId: modVipCard,
    children: [vipDiamondWrap, vipLevelTitle],
  ...
  reg(vipDiamondIcon, "content.icon", "钻石图标", {
    ...
    props: { src: TABLER("diamond"), color: "#C9A227", size: "20px", link: "" },
    bindings: {},
  });
  ...
  reg(vipLevelTitle, "content.text", "VIP 等级标题", {
    ...
    parentId: vipLevelRow,
    children: [],
    wrapperStyle: wsBase({ widthMode: "fill", contentAlign: { horizontal: "left", vertical: "center" } }),
    props: {
      content: "<p>VIP 1</p>",
  ...
  const benefitsDefault = MEMBER_BENEFITS.map((b) => ({
    title: b.title,
    subtitle: b.subtitle,
    iconSrc: b.icon,
  }));
  const benefitsMeta = {
    defaultItems: benefitsDefault,
    meta: {
      label: "会员权益列表",
      description: "权益卡内 2 列栅格，固定 5 项。",
      itemFields: [
  ...
  MEMBER_BENEFITS.forEach((item, i) => {
    addBenefitCell(benefitCellIds[i], benefitsGrid, item, "memberBenefits", i, benefitsMeta);
  });

  reg(benefitsGrid, "layout.container", "权益列表", {
    id: benefitsGrid,
    type: "layout",
    parentId: modVipCard,
    children: benefitCellIds,
    wrapperStyle: wsBase(),
    props: {
      direction: "vertical",
      gapMode: "fixed",
  ...
  reg(joinBtn, "action.button", "加入按钮", {
    id: joinBtn,
    type: "button",
    parentId: modVipCard,
    children: [],
    ...
    props: {
      text: "Join Now",
      link: "https://example.com/join-vip",
      buttonStyle: {
        widthMode: "hug",
    ...
    bindings: {
      ...
      "props.link": {
        slotId: "joinButtonUrl",
        mode: "variable",
        valueType: "url",
        defaultValue: "https://example.com/join-vip",
        allowExternal: true,
        fieldKind: "content",
        label: "加入按钮链接",
      },
  ...
  return {
    schemaVersion: "3.0.0",
    emailId: "member_vip_join",
    templateId: "member_vip_join",
    templateVersion: 1,
    locale: "en-US",
    rootBlockId: ROOT_ID,
    blockMeta,
...
function writePayload() {
  return {
    ...
    values: {
      ...
      memberBenefits: MEMBER_BENEFITS.map((b) => ({
        title: b.title,
        subtitle: b.subtitle,
        iconSrc: b.icon,
      })),
      joinButtonText: "Join Now",
      joinButtonUrl: "https://example.com/join-vip",
...
function writeTokenPresets() {
  return {
    ...
    presets: {
      default: {
        label: "VIP 会员加入预设",
        description: "黑金尊享主题、奶油权益卡与标准间距档位。",
...
function writeMeta() {
  return {
    displayName: "VIP 会员加入（Join VIP 学习模板）",
    description:
      "店铺 Logo、VIP 主视觉、入会即时奖励、会员权益卡（等级/进度/权益/Join Now）、账户与联系；样式经 tokenPresets 统一管理。",
    source: "agent",
    status: "draft",
        ...
        fields: [
          {
            ...
            target: {
              kind: "templatePath",
              path: "blocks.mj-root.props.width",
            },
          },
        ],
      },
...
const norm = spawnSync(
  ...
  [
    ...
    "--write",
    "data/emails/member-vip-join/layouts/default/template.json",
  ],
  { cwd: ROOT, stdio: "inherit" },
);
if (norm.status !== 0) process.exit(norm.status ?? 1);

console.log(`Wrote member-vip-join template to ${OUT_DIR}`);
