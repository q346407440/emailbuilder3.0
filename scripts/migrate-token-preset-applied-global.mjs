 ...
 *
 * 当前仓库约定：
 * - `on-cart-abandon-2`：与 `data/token-presets/public-neutral-saas.json` 的 default.tokens 对齐，
 *   设置 `appliedGlobalPresetId: "public-neutral-saas"`；保留本预设的 label / description。
 * - 其他邮件：若 active 预设的 tokens 与某一公共预设完全一致，则写入对应 appliedGlobalPresetId；
 *   否则不写（仍为「本邮件」语义）。
 *
 * 用法：
...
/** 与 public-neutral-saas 对齐并固定关联的邮件（当前仅弃购 2） */
const FORCE_ALIGN_NEUTRAL_SAAS = new Set(["on-cart-abandon-2"]);
...
function migrateOne(emailKey, globals) {
  ...
  if (FORCE_ALIGN_NEUTRAL_SAAS.has(emailKey)) {
    const src = globals.get("public-neutral-saas");
    if (!src) {
      console.error("缺少 data/token-presets/public-neutral-saas.json，无法对齐 on-cart-abandon-2");
      process.exit(1);
    }
    const srcTokens = structuredClone(pickActiveTokens(src));
    if (!srcTokens) {
      console.error("public-neutral-saas 缺少 tokens");
      process.exit(1);
    }
    if (stableTokensJson(preset.tokens) !== stableTokensJson(srcTokens)) {
      preset.tokens = normalizeTokenPresetTokens(srcTokens);
      changed = true;
      log("已将 default.tokens 与 public-neutral-saas 对齐（保留 label/description）");
    }
    if (tp.appliedGlobalPresetId !== "public-neutral-saas") {
      tp.appliedGlobalPresetId = "public-neutral-saas";
      changed = true;
      log("已设置 appliedGlobalPresetId = public-neutral-saas");
    }
    if (!changed) log("已与 public-neutral-saas 关联且数值一致，无需变更");
  } else {
