...
| easy-email-restore-ast-locate | [.claude/skills/easy-email-restore-ast-locate.md](.claude/skills/easy-email-restore-ast-locate.md) | 由 Inspector「复制定位信息」（模板路径 + 区块 ID）反查 AI 以图还原日志中的 restore-ast.json AST 节点：解析 logs/restore-ast-*/07-block-id-map.json，输出 astPath 与对应 JSON 片段。 当用户粘贴区块定位文案、问「这个模块 AI 还原的 restore json 在哪」「astPath」「block-id-map」 或排查以图还原某区块语义问题时使用；配套脚本 scripts/restore-ast-locate.mjs。 |
