...
describe("blockFieldClassification —— 通用 wrapperStyle 字段", () => {
  it("padding / borderRadius / border / backgroundColor → style", () => {
    assert.equal(classifyField("layout", "wrapperStyle.padding.unified"), "style");
    assert.equal(classifyField("button", "wrapperStyle.borderRadius.radius"), "style");
...
describe("blockFieldClassification —— button（含灰色字段定调）", () => {
  ...
  it("buttonStyle.* → style", () => {
    ...
    assert.equal(classifyField("button", "props.buttonStyle.borderRadius.radius"), "style");
