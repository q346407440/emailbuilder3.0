...
- button   必填 label；可选 href / tone / radius / width(fill|hug) / height(hug|relaxed)   // 仅真实 CTA，见硬规则
...
- **实心可点击行动色块 / 胶囊** → \`button\`；**分隔线 + 粗体文字链**外观 → \`divider\` + \`text\`，见硬规则 5。
...
C) 分隔线文字链（非 CTA，勿用 button）与通栏 CTA 对照：
...
export function buildRestoreAstUserText(p: { emailKey: string; locale: string }): string {
  return [
    ...
    "- 真实 CTA 才用 button：通栏大条 width:\"fill\"；小胶囊不写 width；按钮明显偏高写 height:\"relaxed\"，常规不写 height。",
