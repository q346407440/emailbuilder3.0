...
describe("fetchTemplatesAndValidatePayload", () => {
  ...
  it("其它可见版式读取失败时只记录失败，不阻断当前版式校验", async () => {
    const manifest: LayoutManifest = {
      schemaVersion: "1.0.0",
      activeLayoutVariantId: "default",
      variants: [
        { id: "default", label: "默认", publishStatus: "published" },
        { id: "broken", label: "坏版式", publishStatus: "draft" },
      ],
    };
    const tpl: EmailTemplate = {
      schemaVersion: "1.0.0",
    ...
    const payload: EmailPayload = { schemaVersion: "1.0.0", slots: {}, values: {} };
    const failures: Array<{ layoutVariantId: string; message: string }> = [];
    ...
    const issues = await fetchTemplatesAndValidatePayload(
      ...
      tpl,
      async () => {
        throw new Error("模板校验未通过");
      },
      (failure) => failures.push(failure)
    );

    assert.equal(issues.length, 0);
    assert.deepEqual(failures, [{ layoutVariantId: "broken", message: "模板校验未通过" }]);
  });
});
