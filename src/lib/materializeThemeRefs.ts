...
export const INSERT_THEME_LITERAL_FALLBACKS: Record<string, string> = {
...
export function resolveThemeRefLiteral(
  tokenPath: string,
...
/** 将对象树中的 `$themeRef` 递归展开为字面量。 */
export function deepMaterializeThemeRefs(
  value: unknown,
  tokenPresets: TokenPresets | null | undefined
): unknown {
  if (isThemeRef(value)) {
