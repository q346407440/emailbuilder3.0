...
test("applyContractIssueAutofix heightMode：父 hug 时子 height fill 修复", () => {
  const template = makeTemplate([
    {
      id: "p2",
      type: "layout",
  ...
  const result = applyContractIssueAutofix({
    ...
    tokenPresets: null,
    issues: [
      "blocks.c2.wrapperStyle.heightMode: 父级 layout/image 高度模式为 hug 时，子级不允许使用 height fill（循环依赖）；请改为 hug 或 fixed",
    ],
  });
