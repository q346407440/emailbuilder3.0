...
import { readConfigTargetValue } from "./configSchemaTargets";
...
export function collectMasterValidationIssues(
  ...
  return [
    ...
    ...validateConfigSchema(master.configSchema, template),
  ];
}
...
export function parseBlockMaster(raw: Record<string, unknown>): BlockMaster | null {
  ...
  if (!raw.blocks || !raw.blockMeta || !raw.configSchema) return null;
  return raw as unknown as BlockMaster;
}

export function parseSectionMaster(raw: Record<string, unknown>): SectionMaster | null {
  if (typeof raw.masterId !== "string" || typeof raw.name !== "string") return null;
  if (!raw.blocks || !raw.blockMeta || !raw.configSchema || typeof raw.rootBlockId !== "string") {
    return null;
  }
  return raw as unknown as SectionMaster;
}

export function readMasterFieldValue(
  template: EmailTemplate,
  field: ConfigSchema["scopes"][number]["fields"][number]
): unknown {
  return readConfigTargetValue(field.target, {
    template,
