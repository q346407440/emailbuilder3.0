...
export function DataGroupBindEntryModal({
  ...
  return (
    <ShopSectionModal
      ...
      destroyOnClose
      maskClosable={false}
      wrapClassName="text-body-var-pill-modal-wrap repeat-region-bind-modal-wrap shop-section-modal-wrap"
    ...
    >
      <Field label="选择变量" className="inspector-field--modal-table">
        {candidates.length === 0 ? (
          <p className="text-body-var-pill-modal__empty">当前没有可用的列表或对象变量，请先在数据变量面板添加。</p>
        ) : (
          <>
            {nestedParentLabel ? (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="text-body-var-pill-modal__td text-body-var-pill-modal__td--label">
                      <span title={isParentNested ? candidate.description : undefined}>{candidate.label}</span>
                    </td>
